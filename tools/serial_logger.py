...
def decode_imu_sample(sample_bytes):
    return struct.unpack("<BIIhhhhhhH", sample_bytes)
        ...
        if b1[0] == START1:
            ...
            if b2 and b2[0] == START2:
                ...
                if count > 0:
                    ...
                    node_id, seq, time_us, ax, ay, az, gx, gy, gz, flags = decode_imu_sample(first_sample)

                    print(
                        f"Logged {count} samples | "
                        f"node={node_id} seq={seq} ax={ax} ay={ay} az={az} flags={flags}"
                    )
